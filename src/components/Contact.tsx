...
const Contact = () => {
      ...
      item: [
        {
          ...
          paysItem: [
            {
              icon: <Phone size={20} className="text-accent flex-shrink-0" />,
              title: "Téléphone",
              value: "+1 (450) 933-3335"
            },
            {
              icon: <MapPin size={20} className="text-accent flex-shrink-0" />,
              title: "Adresse",
      ...
      item: [
        {
          ...
          paysItem: [
            {
              icon: <Mail size={20} className="text-accent flex-shrink-0" />,
              title: "Email",
  ...
  return (
    <section id="contact" className="section-padding px-[5%] md:px-[5%] relative">
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-gradient-to-b from-tech-dark dark:from-background via-tech-dark/95 dark:via-background/95 to-tech-dark dark:to-background"></div>
        <img
      ...
      <div className="w-full relative z-0">
        <motion.div
          ...
          viewport={{ once: true }}
          className="text-center flex items-center flex-col gap-3.5 w-full mb-8"
        >
          <div className="inline-block px-6 py-1 bg-white/5 text-primary rounded-full text-xs font-medium border border-white/10">
            Contact
          </div>
          <div className="space-y-1.5">
            <h2 className="heading-md md:heading-lg font-title text-white">Nous Contactez</h2>
            <p className="text-lg w-3/2 text-white/70">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
          <Card className="glass-effect border-white/10 px-5 md:px-7 py-7 card-hover rounded-xl overflow-hidden">
            <div className="absolute -top-32 -right-0 w-60 h-64 bg-accent/20 rounded-full blur-[80px] pointer-events-none"></div>
            <div className="relative z-0">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label htmlFor="name" className="text-sm font-medium mb-1 flex items-center">
                  ...
                  <Input
                    ...
                    className="glass-effect border-tech-dark/20 dark:border-white/20 bg-tech-dark/5 dark:bg-white/5 focus:border-accent/50 focus:accent-glow"
                  />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="email" className="text-sm font-medium mb-1 flex items-center">
                    ...
                    <Input
                      ...
                      className="glass-effect border-tech-dark/20 dark:border-white/20 bg-tech-dark/5 dark:bg-white/5 focus:border-accent/50"
                    />
                  </div>

                  <div>
                    <label htmlFor="phone" className="text-sm font-medium mb-1 flex items-center">
                ...
                <div>
                  <label htmlFor="message" className="text-sm font-medium mb-1 flex items-center">
          ...
          <div className="space-y-4">
            <Card className="glass-effect border-white/10 px-5 md:px-7 pt-6 pb-4 card-hover rounded-xl relative overflow-hidden">
                ...
                <div className="space-y-1.5">
                  {contactInfo.map((info, index) => (
                    info.libele === "bloc" ?
                      <div key={index} className="grid grid-cols-1 md:grid-cols-2 py-1 gap-4 border-b md:border-b-0 border-primary/40">
                        {info.item.map((x, index) =>
                          <div key={index} className="flex flex-col gap-1.5 border-r-0 md:border-r md:pr-1 border-primary/30 last-of-type:border-none">
                            <h4 className="font-bold underline underline-offset-4 font-title mb-1.5">{x.pays}</h4>
                            {x.paysItem.map((y, indd) =>
                              <div key={indd} className="flex gap-3 py-1 justify-start items-center group hover-scale">
                                {y.icon}
                                <div className="w-[calc(100%-2.75rem)]">
                                  <p className="text-muted-foreground text-[0.9rem] md:text-[0.95rem] group-hover:text-white transition-colors">{y.value}</p>
                                </div>
                              </div>
                            )}
                          </div>
                      ...
                      <div key={index} className="flex w-full md:w-fit whitespace-nowrap mt-2 mx-auto items-center group hover-scale">
            ...
            <Card className="glass-effect border-white/10 p-7 h-[194px] card-hover relative overflow-hidden rounded-xl">
